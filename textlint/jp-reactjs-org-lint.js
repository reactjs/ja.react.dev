...
module.exports = context => {
  return {
    ...
    [context.Syntax.Paragraph]: node => {
      ...
      enforceQuestionSpace(node, text, context);
...
const enforceQuestionSpace = (node, text, context) => {
...
const enforceZenHanSpace = (node, text, context) => {
  ...
  if (hanZen.exec(text) || zenHan.exec(text)) {
    context.report(
      node,
      new context.RuleError(
        '全角文字と半角英数字とが隣接しています。半角スペースを挿入してください。',
      ),
    );
  }
};

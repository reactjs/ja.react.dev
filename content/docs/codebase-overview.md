...
これらの規約をどの React アプリケーションにも推奨するというわけではありません。それらの多くは歴史的な理由で存在し、時間とともに変化する可能性があります。
...
**警告は `warning` の条件が `false` の時に表示されます。**

警告の出力条件についての 1 つの考え方は条件は例外的な状況よりも通常の状況を反映すべきだということです。
...
警告は開発時にのみ有効になります。プロダクションでは、完全に取り除かれています。取り除くのを禁止する必要がある場合は、代わりに `invariant` モジュールを使用してください：
...
コードベースには複数の依存性注入ポイントがあります。将来的には、動的注入メカニズムを取り除き、ビルド中にすべてのピースを静的に配置する予定です。
...
異なるレンダラ間でコードの一部を共有することでこの課題を解決します。React のこの箇所を「リコンサイラ (reconciler, "突き合わせ/擦り合わせ機能")」と呼んでいます。`setState()` のような更新がスケジュールされると、突き合わせ処理ではツリー内のコンポーネントで `render()` を呼び出して、コンポーネントをマウント、更新、もしくはアンマウントします。

...
一般的な React のデータフローでは、[props](/docs/components-and-props.html) は親コンポーネントがその子要素とやりとりする唯一の方法です。 子要素を変更するには、新しい props でそれを再レンダリングします。 ただし、一般的なデータフロー以外で子要素を強制的に変更する必要がある場合もいくつかあります。 変更される子要素は React コンポーネントのインスタンスか、DOM 要素です。 どちらの場合も、React は他の方法を提供します。

### いつ Refs を使うか

Refs に適した使用例は以下の通りです。

* フォーカス、テキストの選択、メディアの選択の管理
* アニメーションの発火

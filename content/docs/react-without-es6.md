...
時には同じ機能が全く異なるコンポーネント間で共有されることがあります。これらは [cross-cutting concerns](https://en.wikipedia.org/wiki/Cross-cutting_concern) と呼ばれることがあります。 `createReactClass` であれば、それ用にレガシーな `mixins` 機能を使うことができます。
